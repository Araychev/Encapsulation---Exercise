...
namespace PizzaCalories
{
    public class Dough
    {
        ...
        {
            var ftObject = allowedFlourTypes.First(ft =>
                (string) ft.GetType().GetProperty("FlourType")?.GetValue(ft, null) == this.FlourType.ToLower());
            var ftModifier = (double) ftObject.GetType().GetProperty("CaloriesModifier").GetValue(ftObject, null);

            var btObject = allowedBakingTechniqueTypes.First(bt =>
                (string) bt.GetType().GetProperty("BakingTechnique")?.GetValue(bt, null) ==
                this.BakingTechnique.ToLower());
